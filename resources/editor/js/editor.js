...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			...
			var container = $('<div/>',{'class':"col-sm-12 tabbable"});
			
			var navTabs = $('<ul/>',
									{ class: "nav"
							}).append($('<li/>',
										{ class:"nav-item"
									}).append($('<a/>',{ class:"nav-link active",
											"href":"#uploadImageBar",
											"data-toggle":"tab"
										}).html("De Computadora")
							)).append($('<li/>',
										{ class:"nav-item"}).append($('<a/>',{ class:"nav-link",
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					reader.onload = (function(imageFile){
						return function(e){
							//Render Thumnails
							var li = $('<li/>',{class:"col-xs-12 col-sm-12 col-md-12 col-lg-12"});
							var a = $('<a/>',{
			...
			}).appendTo($("<span/>", {class:"input-group-btn btn-sm form-control-button-right"}).appendTo(getImageURL));
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													if(navigator.userAgent.match(/MSIE/i)){
														var imageStr = '<img width="50%" src="'+$('#imageList').data('current')+'"/>'
													...
													methods.showMessage.apply(this,["imgErrMsg","Por favor seleccione una imagen"]);
													return false;
												}
